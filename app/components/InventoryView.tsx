...
import { Empty, shareWhatsApp } from "@/app/components/DashboardCards";
...
function shareInventoryItem(item: InventoryItem) {
  const lines = [`🏠 ${item.name}`];
  if (item.brand || item.model) lines.push([item.brand, item.model].filter(Boolean).join(" · "));
  if (item.location) lines.push(`📍 ${item.location}`);
  if (item.serial_number) lines.push(`S/N: ${item.serial_number}`);
  if (item.cost) lines.push(`💰 ${item.cost}`);
  if (item.warranty_expiry) lines.push(`🛡 אחריות עד ${formatShortDate(item.warranty_expiry)}`);
  if (item.specs) {
    const specLines = Object.entries(item.specs).map(([k, v]) => `${k}: ${v}`);
    if (specLines.length) lines.push("", ...specLines);
  }
    ...
    >
      ...
      <div className="flex gap-4 items-start">
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
          <button
            onClick={onShare}
            className="w-8 h-8 rounded-lg flex items-center justify-center text-muted hover:text-green-600 hover:bg-green-500/10 transition-colors"
            title="שתף בוואטסאפ"
          >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      {categoryItems.map((item) => (
                        <ItemCard
                          key={item.id}
                          item={item}
                          onShare={() => shareInventoryItem(item)}
